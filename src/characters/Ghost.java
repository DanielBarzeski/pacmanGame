...
public class Ghost extends GameCharacter {
    ...
    public void draw(Graphics g) {
        updateBounds();
        g.drawImage(
                getSprite().getSubimage(getSpriteBounds().x, getSpriteBounds().y, getSpriteBounds().width, getSpriteBounds().height),
                getLocation().x * Constants.CELL_SIZE + (DELAY - 7) * getCurrentDirection().x * Constants.CELL_SIZE / 8,
                getLocation().y * Constants.CELL_SIZE + (DELAY - 7) * getCurrentDirection().y * Constants.CELL_SIZE / 8,
